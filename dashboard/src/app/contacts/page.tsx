...
import { getTags, getContacts, deleteContact } from '@/app/actions/contacts';
...
export default function ContactsPage() {
  ...
  const fetchContactsData = useCallback(async () => {
    setLoading(true);
    try {
      const { data, totalCount: count } = await getContacts(page, search);
      setTotalCount(count);
  ...
  async function openEditForm(contact: ContactWithTags) {
    ...
    // map the nested tags array to what the form expects
    const mappedTags = (contact.tags || []).map((t: any) => ({ 
      id: contact.id + t.id, 
      contact_id: contact.id, 
      tag_id: t.id 
    }));
    setEditContactTags(mappedTags);
    setFormOpen(true);
  }
