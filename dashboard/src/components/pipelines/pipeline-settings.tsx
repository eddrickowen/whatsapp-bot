...
import { renamePipeline, upsertStages, addStage, deleteStage, deletePipeline } from "@/app/actions/pipelines";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    try {
      await Promise.all([
        renamePipeline(pipeline.id, name.trim()),
        upsertStages(pipeline.id, stageRows)
      ]);
    } catch(error) {
  ...
  async function handleAddStage() {
    ...
    if (!trimmed) return;
    try {
      const stage = await addStage(pipeline.id, trimmed, newStageColor, localStages.length);
      setLocalStages([...localStages, stage as PipelineStage]);
    } catch(error) {
