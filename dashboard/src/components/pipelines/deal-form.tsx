...
import { getContactsSimple, createDeal, updateDeal, deleteDeal } from "@/app/actions/pipelines";
import type {
  Contact,
  Conversation,
  Deal,
...
export function DealForm({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const c = await getContactsSimple();
        if (cancelled) return;
  ...
  async function handleSave() {
    ...
    try {
      if (deal) {
        ...
        await createDeal({ ...payload, status: "open" });
      }
    } catch (error) {
  ...
  async function handleStatusChange(status: DealStatus) {
    if (!deal) return;
    try {
      await updateDeal(deal.id, { stage_id: deal.stage_id, status });
    } catch(error) {
